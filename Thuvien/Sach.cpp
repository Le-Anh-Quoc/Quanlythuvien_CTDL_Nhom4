...
//dinh nghia
Sach::Sach(string _masach, string _tensach, string _tacgia, string _nhaxuatban, string _giaban, string _namphathanh, string _sotrang, string _ngaynhapkho, string _tinhtrangsach)
{
	masach = _masach;
	tensach = _tensach;
	tacgia = _tacgia;
...
void Sach::cap_nhat_sach(Sach arr[1000], int& n, bool f)
{
	bool flag = true;
...
void Sach::xoaSach(Sach arr[1000], int& n)
{
	string id_xoa;
	bool flag = false;
...
int Sach::timSach(Sach arr[1000], int sl, string id)
{
...
void Sach::layThongTinDSSach(Sach arr[1000], int& sl)
{
	fstream f;
	int i = 0;
	sl = 0;
...
void Sach::tinhTrangSach(Sach arr[1000], int sl, string id, string cd)
{
	for (int index = 0; index < sl; index++) {
		if (arr[index].masach == id) {
			//doi tinh trang sach
